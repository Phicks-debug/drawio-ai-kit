...
Resolve icons through `index.mjs` with a `databricks …` query. Databricks has no group/container stencils — draw containers with `frame`/`stage`/`band`; the icons carry identity. Accent red `#FF3621` on borders only, never as a fill (clean house style).
...
Use this canonical Databricks reference layout and adapt it to the requested system.
...
Icon vocabulary (all merged into the `databricks` pack; resolve with a `databricks …` query through `index.mjs`):
...
Use the "Big Book of MLOps" layout described below.
