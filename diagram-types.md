...
Use a junction only for at least three equivalent services on one side. For fan-out, link `source → branch("split") → same-type targets`; for fan-in, link `same-type sources → merge("join") → target`. Keep different service types on individual links and keep at most four total connections per icon side.
